largest_box_panel <- function(df_analysis,frame_paths) {
  list(
    ui = tabPanel(
      "Largest Box Preview",
      fluidPage(
    ...
    server = function(input, output, session) {
      ...
      largest_per_id <- reactive({
        ...
        out_list <- lapply(id_list, function(df_id) {
          ...
          areas <- apply(df_id[, c("x1","y1","x2","y2","x3","y3","x4","y4")], 1, function(r) {
            ...
            poly <- tryCatch(sf::st_polygon(list(rbind(pts, pts[1,]))),
      ...
      output$id_selector_ui <- renderUI({
        req(largest_per_id())
        selectInput("selected_id", "Choose id:",
                    choices = unique(largest_per_id()$id))
      })
      
      #### Auto-select first ID and slider ####
      observeEvent(df_analysis(), {
        df <- df_analysis()
        req(df)
        ids <- unique(df$id)
        if (length(ids) > 0) {
          ...
          if (length(first_frame)) {
            idx <- which.min(abs(as.numeric(gsub(".*_(\\d+)\\.tif$", "\\1",
                                                 basename(frame_paths()))) - first_frame))
            if (length(idx) == 1) updateSliderInput(session, "frame_idx", value = idx)
          }
        }
      })
      
      #### Slider moves with ID ####
      ...
      output$largest_box_plot <- renderPlot({
        ...
        pts <- matrix(as.numeric(row[c("x1","y1","x2","y2","x3","y3","x4","y4")]),
                      ncol = 2, byrow = TRUE)
        ...
        par(xaxs = "i", yaxs = "i")
        plot(NA, xlim = x_range, ylim = rev(y_range), xlab = "", ylab = "",
             axes = FALSE, asp = 1,
        ...
        req(df)
        first_frame <- df$frame_num[df$id == input$selected_id & df$prop_type == "first"]
        border_color <- if (length(first_frame) &&
                            first_frame == as.numeric(gsub(".*_(\\d+)\\.tif$", "\\1",
                                                           basename(frame_file)))) "purple" else "red"
        polygon(pts[,1], pts[,2], border = border_color, lwd = 2)
      })
      
      #### Move first appearance ####
      observeEvent(input$move_first_btn, {
        req(df_analysis, input$selected_id, input$frame_idx, input$stats_file)
        df <- df_analysis()
        ...
        if (length(idx) == 0) idx <- which(df$id == id)[1]
        ...
        showNotification(sprintf("Moved first appearance of id %s to frame %d", id, frame_num),
                         type = "message")
      })
    }
  )
